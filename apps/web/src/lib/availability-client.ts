...
export async function fetchAvailabilityStatusBatched(
  type: "movie" | "tv",
  ids: number[],
  batchSize: number = 50
) {
  const unique = Array.from(new Set(ids.filter(id => Number.isFinite(id) && id > 0)));
  if (unique.length === 0) {
    return {} as Record<number, "available" | "partially_available" | "unavailable">;
  }
  const chunks: number[][] = [];
  for (let i = 0; i < unique.length; i += batchSize) {
    chunks.push(unique.slice(i, i + batchSize));
  ...
  return results.reduce<Record<number, "available" | "partially_available" | "unavailable">>(

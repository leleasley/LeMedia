...
const baseCacheDirectory =
  process.env.IMAGE_CACHE_DIR || path.join(process.cwd(), ".next/cache/images");
...
class ImageProxy {
  public static async clearCache(key: string) {
    ...
    try {
      ...
      if (e?.code === "ENOENT") {
        logger.error("Directory not found", e, { label: "Image Cache" });
      } else {
        logger.error("Failed to read directory", e, { label: "Image Cache" });
      }
    }
  ...
  private static async getDirectorySize(dir: string): Promise<number> {
    try {
      ...
      const paths = files.map(async (file) => {
        ...
        if (file.isDirectory()) return await ImageProxy.getDirectorySize(filePath);
  ...
  public async getImage(path: string, fallbackPath?: string): Promise<ImageResponse> {
  ...
  public async clearCachedImage(path: string) {
    ...
    try {
      ...
      logger.error("Error checking cache directory existence", e, { label: "Image Cache" });
      return;
    }
  ...
  private async set(path: string, cacheKey: string): Promise<ImageResponse | null> {
    try {
      const directory = join(this.getCacheDirectory(), cacheKey);
